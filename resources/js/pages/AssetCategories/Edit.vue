<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6">
      <Heading :title="t('asset_categories.edit')" />
      ...
      <div class="max-w-2xl">
        <form @submit.prevent="submit" class="space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  <option 
                    ...
                    :value="parent.id"
                  >
                    {{ '—'.repeat(parent.depth || 0) }} {{ parent.name }}
                  </option>
                </select>
          ...
          <!-- Danger Zone -->
          <Card v-if="category.children && category.children.length > 0" class="border-red-200">
            <CardHeader>
              <CardTitle class="text-red-700">{{ t('asset_categories.form.danger_zone') }}</CardTitle>
              <CardDescription class="text-red-600">
                {{ t('asset_categories.form.danger_zone_description') }}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div class="space-y-4">
                <div class="p-4 bg-red-50 rounded-lg">
                  <p class="text-sm text-red-700">
                    {{ t('asset_categories.form.has_children_warning', { count: category.children.length }) }}
                  </p>
...
const breadcrumbs = computed((): BreadcrumbItem[] => [
  ...
  {
    title: t('asset_categories.edit'),
    href: `/asset-categories/${props.category.id}/edit`,
  },
])

// Available parents (excluding self and descendants)
const availableParents = computed(() => {
  return props.parentCategories.filter(parent => parent.id !== props.category.id)
})
...
const form = useForm({
  ...
  parent_id: props.category.parent_id || '' as string
})
